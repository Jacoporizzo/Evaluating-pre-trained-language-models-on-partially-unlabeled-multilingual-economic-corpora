...
(pn.ggplot(bars_df, pn.aes(x = 'index', y = 'total'))
     ...
     + pn.labs(y = 'Absolute frequency', title = "German goldstandards' true labels")
     + pn.theme(axis_text_x = pn.element_text(angle = 90)))

bars_df['proportion'] = bars_df['total']/sum(bars_df['total'])
(pn.ggplot(bars_df, pn.aes(x = 'index', y = 'proportion'))
     + pn.geom_col(color = 'blue', fill = 'blue')
     + pn.labs(y = 'Relative frequency', title = "German goldstandards' true labels")
     + pn.theme(axis_text_x = pn.element_text(angle = 90)))
...
(pn.ggplot(english_bars_df, pn.aes(x = 'index', y = 'total'))
     + pn.geom_col(color = 'blue', fill = 'blue')
     + pn.labs(y = 'Absolute freqeuncy', title = "English goldstandards' true labels")
     + pn.theme(axis_text_x = pn.element_text(angle = 90)))
...
(pn.ggplot(english_bars_df, pn.aes(x = 'index', y = 'proportion'))
     + pn.geom_col(color = 'blue', fill = 'blue')
     + pn.labs(y = 'Relative freqeuncy', title = "English goldstandards' true labels")
     + pn.theme(axis_text_x = pn.element_text(angle = 90)))